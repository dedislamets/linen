...
class Linenkotor extends CI_Controller
{
    ...
    public function edit($id)
    {
        if ($this->admin->logged_id()) {
            ...
            $this->db->select("id,/*STR_TO_DATE(TANGGAL, '%d/%m/%Y')*/ TANGGAL,NO_TRANSAKSI,PIC,STATUS,TOTAL_BERAT,TOTAL_BERAT_REAL,TOTAL_QTY,KATEGORI,F_INFEKSIUS, penerima, signature, tgl_terima");
            $this->db->from("linen_kotor");
    ...
    public function savesignature()
    {
        ...
        $imgfile = $this->input->post('id_kotor') . '.' . $image_type;
        ...
        $this->db->where(
            array(
                "id" => $this->input->post('id_kotor'),
            )
        );
        $this->db->update('linen_kotor');


        $keluar = $this->admin->get_array('linen_kotor', array('id' => $this->input->post('id_kotor')));
        $msg = 'No #' . $keluar['NO_TRANSAKSI'] . ' sudah diterima';
        ...
        foreach ($user_data as $key => $value) {
            $data_notif = array(
                'short_msg'   => $msg,
                'long_msg'    => $msg,
                'url'         => 'linenkotor/detail/' . base64_encode($this->input->post('id_kotor')),
        ...
        redirect(site_url('linenkotor/detail/' . base64_encode($this->input->post('id_kotor'))));
    }
