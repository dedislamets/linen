...
class Cucibersih extends CI_Controller {
    ...
    {
      if(CheckMenuRole('cucibersih')){
        redirect("errors");
      }
			$data['title'] = 'Cuci Simpan';
			$data['main'] = 'linen/cucibersih';
			$data['js'] = 'script/cucisimpan';
  ...
  public function dataTable()
  {
      ...
      foreach($pengguna->result() as $r)
      {
          ...
          $data[] = array( 
                      ...
                      $status,
                      '<a href="cucibersih/edit/'.$r->id.'"  class="btn btn-warning btn-sm "  >
                        <i class="icofont icofont-edit"></i>Data Cuci
                      </a>|<a href="cucibersih/proses/'.$r->id.'"  class="btn btn-info btn-sm "  >
                        <i class="icofont icofont-edit"></i>Data Penyimpanan
                      </a>'. $tbl_del ,
                 );
      }
  ...
  public function edit($id){
    if($this->admin->logged_id())
    {
      $data['title'] = 'Edit Cuci Linen & Simpan';
      $data['main'] = 'linen/cucibersih-edit';
  ...
  public function create(){
    if($this->admin->logged_id())
    {
      $data['title'] = 'Tambah Cuci Simpan';
      $data['main'] = 'linen/cucibersih-create';
      $data['js'] = 'script/cucisimpan-create';
      $data['modal'] = 'modal/kotor'; 
      $data['mode'] ='new';
      $data['totalrow'] = 0;
  ...
  public function proses($id){
    if($this->admin->logged_id())
    {
      $data['title'] = 'Tambah Linen Masuk Penyimpanan';
      $data['main'] = 'linen/cucibersih-proses';
  ...
  {       
      ...
      if($this->input->post('id_kotor') != "") {
              ...
              for ($i=1; $i <= $total ; $i++) { 
                if(!empty($this->input->post('epc'.$i) )){
                  ...
                  if(!empty($this->input->post('id_detail'.$i) )){
                    ...
                    unset($data_bersih);
                    $data_bersih['no_transaksi'] = $this->input->post('no_transaksi');
                    $data_bersih['epc'] = $this->input->post('epc'.$i);
                    $data_bersih['ruangan'] = $this->input->post('ruangan'.$i);
                    $data_bersih['checked'] = 1;
                    $data_bersih['status_linen'] = 'OK';

                    $this->db->insert('linen_bersih_detail', $data_bersih);
          ...
          $data_bersih['STATUS'] = "BERSIH";

          $result  = $this->db->insert('linen_kotor', $data);
          $last_id = $this->db->insert_id();

          $result_bersih  = $this->db->insert('linen_bersih', $data_bersih);
          $last_id_bersih = $this->db->insert_id();

          $response['id']= $last_id;
          $response['id_bersih']= $last_id_bersih;
          if(!$result){
              print("<pre>".print_r($this->db->error(),true)."</pre>");
          }else{
              $response['error']= FALSE;
