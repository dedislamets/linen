<script type="text/javascript">
   ...
   function scanning(session,QValue,anteana){
     ...
     $('#btn-finish').on('click', function (event) {
     	...
     	if($status) {
     		if(validateBarang()){
     			var link = '<?= base_url(); ?>cucibersih/Save';
     			$.post(link,sParam, function(data){
     				if(data.error==false){	
     					alert("Berhasil disimpan..!");
						// window.location.href="<?= base_url(); ?>linenkotor/edit/"+ data.id
						window.location.href="<?= base_url(); ?>cucibersih";
     ...
     function cancel(val) {
     	...
     	if(id != ""){
     		...
     		if (r == true) {
     			$.get('<?= base_url()?>cucibersih/delete', { id: id, no_transaksi: $("#no_transaksi").val() }, function(data){ 
