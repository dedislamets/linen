<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		if ($("#mode").val() == 'edit') {
			app.mode = 'edit';
			$.get('<?= base_url() ?>linenkeluar/getDetail', {
				id: $("#id_keluar").val()
			}, function(data) {
	...
	$('#btnCetak').on('click', function(event) {
		var id_keluar_encrypted = btoa($("#id_keluar").val());
		window.open("<?= base_url(); ?>cetak?p=keluar&id=" + id_keluar_encrypted, '_blank');
	})
	...
	function validateBarang() {
		...
		if (app.list_request.length > 0) {
			$.each(app.list_scan, function(i, obi) {
				var exist = app.list_request.filter(function(person) {
					return person.jenis == obi.jenis
				});
				if (exist.length == 0) {
					flag = false;
	...
	function cancel(val) {
		...
		if (id != "") {
			...
			if (r == true) {
				$.get('<?= base_url() ?>linenbersih/delete', {
					id: id,
